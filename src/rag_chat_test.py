...
def main():
    ...
    for i, q in enumerate(QUESTIONS, start=1):
        ...
        try:
            ...
            errored += 1
            print(f"❌ Ошибка при запросе к LLM/эмбеддингам: {e}")
            continue
    ...
    print(f"✔ Ответов с ссылками [N]: {passed}")
    print(f"✖ Ответов без ссылок [N]: {failed}")
    print(f"⚠ Вопросов с ошибкой (500/EOF и т.п.): {errored}")
    print("=" * 80)
