...
5. **Отправляет админу для аппрува** ⭐ НОВОЕ
6. Публикует в Telegram после одобрения
7. Dashboard показывает статистику
...
```
RSS Sources → News Agent → RAG (дубликаты) → AI Filter → MCP (лимиты) → Admin Approve → Telegram
                ↓                                            ↓              ↓
...
TELEGRAM_ADMIN_ID=123456789  # Твой user ID для аппрува
```
...
- `TELEGRAM_BOT_TOKEN`
- `TELEGRAM_CHAT_ID`
- `TELEGRAM_ADMIN_ID` (опционально для аппрува)
...
**Терминал 2: Approval Bot** (опционально, для аппрува)
```bash
cd agent
python approval_bot.py
# Обрабатывает кнопки ✅/❌ в Telegram
```

**Терминал 3: News Agent**
```bash
cd agent
python news_agent.py
```

**Терминал 4: Dashboard** (опционально)
```bash
cd dashboard
...
## 🎯 Система аппрува (НОВОЕ!)

### Как работает:

1. **Agent находит новость** → Отправляет админу в Telegram
2. **Админ получает сообщение** с кнопками [✅ Опубликовать] [❌ Отклонить]
3. **Админ нажимает кнопку** → Новость публикуется или отклоняется

### Настройка:

```bash
# С аппрувом (рекомендуется)
export TELEGRAM_ADMIN_ID="123456789"  # Твой user ID
python agent/approval_bot.py          # Запусти бот

# Без аппрува (автопубликация)
unset TELEGRAM_ADMIN_ID  # Не устанавливай admin ID
```

### В Telegram:

```
📰 НОВАЯ НОВОСТЬ ДЛЯ АППРУВА:

🚀 Tesla представила FSD v12!
...

[✅ Опубликовать]  [❌ Отклонить]
```

**Подробнее:** См. [APPROVAL_SYSTEM.md](APPROVAL_SYSTEM.md)
